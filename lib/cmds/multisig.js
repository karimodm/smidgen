...
smidgen multisig reattach <file>
...
function reattach (iota, opts, filename, cb) {
  readAndCheckFile(filename, null, opts, (err, content) => {
    if (err) return cb(err)

    if (!content.current) {
      const err = new Error([
        'Found no current main address, is file finalized already?'
      ].join('\n'))
      err.type = 'EUSAGE'
      return cb(err)
    }

    const mainAddress = content.current.mainAddress
    const transfers = [{
      address: iota.utils.noChecksum(mainAddress),
      value: 0,
      message: '',
      tag: ''
    }]

    generateSeed(iota, {}, (err, seed) => {
      if (err) return cb(err)

      iota.api.sendTransfer(seed, opts.depth, opts.mwm, transfers, (err, transaction) => {
        if (err) return cb(err)

        if (opts.json) {
          return cb(null, { mainAddress: mainAddress })
        }

        const hash = transaction[0].hash

        return cb(null, { hash, mainAddress })
      })
    })
  })
}
...
function cliReattach (filename, cb) {
  if (!filename) {
    const err = new Error('smidgen multisig reattach <file>')
    err.type = 'EUSAGE'
    return cb(err)
  }

  reattach(smidgen.iota, smidgen.config, filename, (err, res) => {
    if (err) return cb(err)

    smidgen.log.info('', `Successfully sent 0i to ${res.mainAddress} from random seed`)
